...
#  This function peakfinder opens the csv file by user selection. Defaults to
# test data1
#  uses a peak detection algorithm
#  From MIT. The values returned in myindices are the indices of peaks in the
# time array.
#  Returns 1 array This function is tested by setting the show = True and
# visually inspecting
def peakfinder(vd):
    """This function calls the peak finder Algorithm used within rightsfromMIT.
    ...
    Raises:
        TypeError: If the array is empty or if 'vd' is an int or str or NaN
        another type is not possible.

    """
    if len(vd) == 0 or isinstance(vd, int) or isinstance(vd, str):
        raise TypeError("Error: Array Passed into peakfinder was Empty or not "
                        "an Array")
...
#  The timefinder function returns all the times that a peak occured by using
# the
#  arrray of peak indices. Returns an array. Only simple test is used because
...
def timefinder(mytime, myindex):
    ...
    Returns:
        ...
        Raises:
            TypeError: If the array is empty or if 'mytime' or 'myindex' is an
            int or str or NaN.
    """
    if len(mytime) == 0 or isinstance(mytime, int) or isinstance(mytime, str):
        raise TypeError("Error: Array Passed into timefinder was Empty or not "
                        "an Array")
    if len(myindex) == 0 or isinstance(myindex, int) or isinstance(myindex,
                                                                   str):
        raise TypeError("Error: Array Passed into timefinder was Empty or not "
                        "an Array")
    if len(myindex) > len(mytime):
        raise ValueError("Error: There are too many values within the array "
                         "that contains indices of beats")
    newtime = []
    for each in myindex:
...
#  avghr Allows the user to select and Avg HR interval based in seconds and
# requires
#  it to be in the range of the data. If not in range it throws a value error
#  To calculate intervals it looks at the distance between elements and waits
# until
#  the interval is matched or exceeded. It then takes the number of beats found
#  in the
#  interval and outputs all the beats found in that interval into an array
#  resulting in array of beats per interval. This is then converted to
# beats/sec ->
...
def avghr(dur, tarr):
    ...
    Raises:
        TypeError: If the values passed in for 'dur' and 'tarr' were empty or
        not valid types.

    """
    if isinstance(dur, str) or np.isnan(dur):
        raise TypeError("Error: Value Passed into avghr was Empty or not the "
                        "correct Type.")
    if len(tarr) == 0 or isinstance(tarr, int) or isinstance(tarr, str):
        raise TypeError("Error: Array Passed into avghr was Empty or not an "
                        "array.")
...
#  durr find the total duration of the data. It must take in the full array of
# times
...
def durr(tarr):
    ...
    """
    if len(tarr) == 0 or isinstance(tarr, int) or isinstance(tarr, str):
        raise TypeError("Error: Array Passed into durr was Empty or not an "
                        "array.")
    f = len(tarr) - 1
...
def findex(volt):
    ...
    if len(volt) == 0 or isinstance(volt, int) or isinstance(volt, str):
        raise TypeError("Error: Array Passed into findex was Empty or not an "
                        "array.")
    mmax = np.max(volt)
    mmin = np.min(volt)
...
# numbeats takes in array of beat indices and find the length to get the total
# beats
# Returns a single value as a np.array. So simple doesn't really need
# significant testing.
def numbeats(beatindex):
    ...
    """
    if len(beatindex) == 0 or isinstance(beatindex, int) or isinstance(
            beatindex, str):
        raise TypeError("Error: Array Passed into beatindex was Empty or not "
                        "an array.")
    mynum = len(beatindex)
